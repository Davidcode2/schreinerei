...
describe("InventoryDetailPage interactions", () => {
  ...
  it("defaults the withdrawal dialog to the active project and submits with site_id", async () => {
    const user = userEvent.setup()
    let withdrawPayload: unknown = null

    server.use(
      http.get(apiPath("/preferences"), () =>
        HttpResponse.json({ active_site_id: "site-2" })
      ),
      http.get(apiPath("/sites"), () =>
        HttpResponse.json([
          { id: "site-2", name: "CNC Vorbereitung", project_type: "internal_workshop" },
        ])
      ),
    ...
    expect(await screen.findByLabelText(/projekt \*/i)).toBeInTheDocument()

    await user.click(screen.getByRole("button", { name: /stück entnehmen/i }))
