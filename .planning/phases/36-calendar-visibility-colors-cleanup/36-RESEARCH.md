# Phase 36 Research — Calendar Visibility, Colors & Cleanup
...
What do we need to know to plan Phase 36 well so the embedded fleet calendar stays clear while selecting, uses deterministic resource colors, and fully replaces the old standalone entry point?

## Existing Implementation Snapshot

### Current embedded calendar state after Phase 35
...
- Phase 35 already delivered two-tap selection and the bottom confirmation sheet; Phase 36 must preserve that flow, not replace it.
...
- Phase 34 kept the standalone route only as a transition path; Phase 36 is the cleanup phase that should end that transition.
