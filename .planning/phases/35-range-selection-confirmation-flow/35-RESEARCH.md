# Phase 35 Research — Range Selection & Confirmation Flow
...
## Constraints From Phase 34 / Project State

- Phase 34 explicitly kept `/fleet` and `/fleet/calendar` on the same `CalendarView` implementation.
- Project state says selection must stay tied to a single resource row.
- Requirements only change the embedded calendar interaction for this phase; removing the standalone entry point and stable colors belong to Phase 36.
...
- Phase 35 only needs a new confirmation experience after calendar range completion, so a dedicated confirmation sheet is a narrower change.
...
- Phase 35 must add pending selection feedback without replacing or hiding those chips.
- Phase 36 owns color improvements; this phase should keep the current reservation rendering intact and layer selection state on top.
