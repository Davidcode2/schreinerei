...
use crate::modules::fleet::domain::{
    ...
    ReservationCreatedPayload, ReservationUpdatedPayload, ReservationCancelledPayload,
};
...
impl FleetService {
    ...
    pub async fn create_reservation(
        ...
        let resource_exists = match create.resource_type {
            ResourceType::Vehicle => {
                self.fleet_repo.find_vehicle_by_id(ctx.tenant_id, VehicleId(create.resource_id))
                    .await?
                    .is_some()
            }
            ResourceType::Tool => {
                self.fleet_repo.find_tool_by_id(ctx.tenant_id, ToolId(create.resource_id))
                    .await?
                    .is_some()
            }
        };
        ...
        // Check availability
        let is_available = self.fleet_repo.check_availability(
            ctx.tenant_id,
            create.resource_type,
            create.resource_id,
            create.start_time,
    ...
    pub async fn update_reservation(
        ...
        let current = self.fleet_repo.find_reservation_by_id(ctx.tenant_id, reservation_id)
            .await?
        ...
        let reservation = self.fleet_repo.update_reservation(ctx.tenant_id, reservation_id, &update).await?;
    ...
    pub async fn cancel_reservation(
        ...
        self.fleet_repo.update_reservation(ctx.tenant_id, reservation_id, &update).await?;
    ...
    pub async fn get_reservation(
        ...
        self.fleet_repo.get_reservation_with_details(ctx.tenant_id, reservation_id)
            .await?
            .ok_or_else(|| AppError::NotFound("Reservation not found".to_string()))
    }
    ...
    pub async fn list_reservations(
        ...
        let reservations = self.fleet_repo.list_reservations(
            ctx.tenant_id,
            filter_user_id,
            resource_type,
            resource_id,
        ...
        for reservation in reservations {
            if let Some(details) = self.fleet_repo.get_reservation_with_details(ctx.tenant_id, reservation.id).await? {
    ...
    pub async fn get_calendar(
        ...
        self.fleet_repo.get_calendar_data(ctx.tenant_id, start_date, end_date, resource_type).await
    }
    ...
    pub async fn check_availability(
        ...
        self.fleet_repo.check_availability(
            ctx.tenant_id,
            resource_type,
            resource_id,
            start_time,
    ...
    pub async fn get_status_by_qr(
        ...
        self.fleet_repo.get_resource_status_by_qr(ctx.tenant_id, qr_code)
            .await?
